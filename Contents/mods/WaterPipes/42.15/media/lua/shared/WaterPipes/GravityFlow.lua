...
-- caller). Water only moves DOWN across floors and pools at the lowest containers first: the component's
-- total is poured into the LOWEST z-level first, equalized within the level, overflowing upward. A
-- single-floor component is exactly the classic per-pool equalization. Conservation-safe: exactly the
-- input water is written back, capped at total capacity.
-- Tap-side reachability lives in the gravity-aware NetworkAccess pass; here we only relocate stored water.
...
function GravityFlow.settle(containers, totalWater, fluidTypeName)
    ...
    -- `carry` is what a vessel did not take, rolled into the next one: either it clamped at its capacity,
    -- or the change was too small for Adapter.writeDescriptorWaterAmount to think it worth a write. Without
    -- it, a settle that declines a hundredth of a litre per vessel loses that, every ten in-game minutes.
